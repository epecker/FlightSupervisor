...
int main() {
    ...
    const string i_base_dir = string(PROJECT_DIRECTORY) + string("/test/input_data/udp_output_fcc/");
    const string o_base_dir = string(PROJECT_DIRECTORY) + string("/test/simulation_results/udp_output_fcc/");
    ...
    do {
        ...
        std::shared_ptr<dynamic::modeling::model> udp_output = dynamic::translate::make_dynamic_atomic_model<UDP_Output, TIME, const char *, const unsigned short, bool>(
                "udp_output", IPV4_FCC, PORT_FCC, false);

        // Instantiate the input readers.
        // One for each input
        std::shared_ptr<dynamic::modeling::model> ir_message =
                dynamic::translate::make_dynamic_atomic_model<Input_Reader_Fcc_Command, TIME, const char *>("ir_message",
                                                                                                           input_file_in.c_str());
        ...
        dynamic::modeling::ICs ics_TestDriver = {
                dynamic::translate::make_IC<iestream_input_defs<message_fcc_command_t>::out, Packet_Builder_Fcc<TIME>::defs::i_data>(
                        "ir_message", "packet_builder"),
                dynamic::translate::make_IC<Packet_Builder_Fcc<TIME>::defs::o_packet, UDP_Output<TIME>::defs::i_message>(
                        "packet_builder", "udp_output")
        };
