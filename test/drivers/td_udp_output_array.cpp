...
template<typename T>
class UDP_Output_Array_Char : public UDP_Output<std::array<char, MAVLINK_PACKET_SIZE>, T> {
public:
    UDP_Output_Array_Char() = default;
    UDP_Output_Array_Char(string address, string port) : UDP_Output<std::array<char, MAVLINK_PACKET_SIZE>, T>(address, port){}
};

int main(int argc, char* argv[]) {
	int test_set_enumeration = 0;

	const string i_base_dir = string(PROJECT_DIRECTORY) + string("/test/input_data/udp_output_array/");
	const string o_base_dir = string(PROJECT_DIRECTORY) + string("/test/simulation_results/udp_output_array/");
	...
	do {
		...
		std::shared_ptr<dynamic::modeling::model> udp_output = dynamic::translate::make_dynamic_atomic_model<UDP_Output_Array_Char, TIME, const char*, const char*>("udp_output", std::move("127.0.0.1"), std::move("23000"));

		// Instantiate the input readers.	
		// One for each input
		std::shared_ptr<dynamic::modeling::model> ir_message =
			dynamic::translate::make_dynamic_atomic_model<Input_Reader_Update_GCS, TIME, const char* >("ir_message", std::move(input_file_in.c_str()));
		...
		dynamic::modeling::ICs ics_TestDriver = {
			...
			dynamic::translate::make_IC<Packet_Builder_GCS<TIME>::defs::o_packet, UDP_Output<array<char, MAVLINK_PACKET_SIZE>, TIME>::defs::i_message>("packet_builder", "udp_output")
		};
